...
if (command == 'جمال') {
let juego = `معدل جمال السيد(ة): ${text.toUpperCase()} بنسبه 100/100\nهيا : ${(100).getRandom()}%`.trim()
await conn.reply(m.chat, juego, m, m.mentionedJid ? { mentions: m.mentionedJid } : {})}

 // ------------------------------------------------------------------------------------------------------------------------------------------------
  
if (command == 'ذكاء') {
let juego = `_*${text.toUpperCase()} ذكي بنسبه ${(100).getRandom()}%*_`.trim()
await conn.reply(m.chat, juego, m, m.mentionedJid ? { mentions: m.mentionedJid } : {})}
...
if (command == 'غباء') {
let juego = `_*${text.toUpperCase()} غبي بنسبه ${(100).getRandom()}%*_`.trim()
await conn.reply(m.chat, juego, m, m.mentionedJid ? { mentions: m.mentionedJid } : {})}
